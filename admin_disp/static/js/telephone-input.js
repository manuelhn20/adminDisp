// Telephone input plugin script (from Telephone-Input-Field-With-Country-Code-Flag-main/script.js)
const countries = [
...
function initTelephoneInput(selectBoxSelector) {
    const container = document.querySelector(selectBoxSelector);
    if (!container) return;
    
    const select_box = container.querySelector('.options');
    const search_box = container.querySelector('.search-box');
    const input_box = container.querySelector('input[type="tel"]');
    const selected_option = container.querySelector('.selected-option div');
    const ol = select_box.querySelector('ol');
    const selected_option_wrapper = container.querySelector('.selected-option');

    if (!select_box || !search_box || !input_box || !selected_option || !ol) {
        console.error('Telephone input: missing DOM elements');
        return;
    }

    // IMPORTANTE: Remover todos los event listeners antiguos clonando y reemplazando
    const new_select_box = select_box.cloneNode(true);
    select_box.parentNode.replaceChild(new_select_box, select_box);
    
    const new_selected_option_wrapper = selected_option_wrapper.cloneNode(true);
    selected_option_wrapper.parentNode.replaceChild(new_selected_option_wrapper, selected_option_wrapper);
    
    // Re-obtener referencias después del cloning
    const updated_select_box = container.querySelector('.options');
    const updated_search_box = container.querySelector('.search-box');
    const updated_input_box = container.querySelector('input[type="tel"]');
    const updated_selected_option = container.querySelector('.selected-option div');
    const updated_selected_option_wrapper = container.querySelector('.selected-option');
    const updated_ol = updated_select_box.querySelector('ol');

    // Limpiar opciones anteriores
    updated_ol.innerHTML = '';
    ...
    // Trigger Iconify to scan and load the new icons
    if (typeof Iconify !== 'undefined') {
        Iconify.scan();
    }

    // Click on flag/code div to toggle dropdown (not on the input)
    updated_selected_option.addEventListener('click', (e) => {
        e.stopPropagation();
        updated_select_box.classList.toggle('active');
        updated_selected_option_wrapper.classList.toggle('active');
        if (updated_select_box.classList.contains('active')) {
            updated_search_box.focus();
        }
    });
    
    // Prevent click on input from toggling dropdown
    updated_input_box.addEventListener('click', (e) => {
        e.stopPropagation();
    });
    ...
    document.addEventListener('click', (e) => {
        if (!container.contains(e.target)) {
            updated_select_box.classList.remove('active');
            updated_selected_option_wrapper.classList.remove('active');
        }
    });

    // Search countries
    updated_search_box.addEventListener('input', (e) => {
        const search_query = updated_search_box.value.toLowerCase();
        const options = updated_ol.querySelectorAll('.option');
        options.forEach(option => {
            const countryName = option.querySelector('.country-name').innerText.toLowerCase();
            const isMatched = countryName.includes(search_query);
            option.classList.toggle('hide', !isMatched);
        });
    });

    // Select option using event delegation
    updated_ol.addEventListener('click', (e) => {
        ...
        updated_selected_option.innerHTML = '';
        updated_selected_option.append(icon, phone_code);
        
        // Store country code in hidden input (without + sign)
        const countryCode = phone_code.innerText.trim().replace('+', '');
        const hiddenInputId = container.id === 'newNumeroLineaSelect' ? 'newNumeroLineaHidden' : 'editNumeroLineaHidden';
        const input_hidden = document.getElementById(hiddenInputId);
        if (input_hidden) {
            input_hidden.value = countryCode;
        }
        
        // No limpiar el input al cambiar de país - mantener el número existente
        updated_input_box.focus();

        updated_select_box.classList.remove('active');
        updated_selected_option_wrapper.classList.remove('active');
        updated_search_box.value = '';
        updated_ol.querySelectorAll('.hide').forEach(el => el.classList.remove('hide'));
        
        // Trigger Iconify to render the new icon
        if (typeof Iconify !== 'undefined') {
            Iconify.scan();
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    initTelephoneInput('#newNumeroLineaSelect');
});

// Hook into modal opening to ensure reinitialization
const origOpenModal = window.openModal;
if (typeof origOpenModal === 'function') {
    window.openModal = function(modalId, ...args) {
        origOpenModal.call(this, modalId, ...args);
        if (modalId === 'modalNewPlane') {
            setTimeout(() => {
                initTelephoneInput('#newNumeroLineaSelect');
                if (typeof Iconify !== 'undefined') Iconify.scan();
            }, 50);
        }
        if (modalId === 'modalEditPlane') {
            setTimeout(() => {
